...
def plot_pd_scnr(results: dict, save_path: str = 'pd_scnr.png'):
    """
    results : dict
        maps ν → (pd_array, pfa_array, scnr_array)
    """
    plt.figure(figsize=(12, 6))
...
class CombinedRadarTester:
    ...
    def evaluate_pd_pfa(self, nu: float, scnr: float = 0) -> tuple:
        ...
        target_pFAs = np.logspace(-5, 0, 10)  
...
class NewCombinedRadarTester:
    ...
    def evaluate_pd_pfa(self, nu: float, snr: float = 15, cnr: float = 15) -> tuple:
        test_dataset_with_targets = RadarDataset(4096, n_targets=4, random_n_targets=False, nu=nu, snr=15, cnr=15)
        test_dataset_no_targets   = RadarDataset(2048, n_targets=0, random_n_targets=False, nu=nu, snr=15, cnr=15)
        test_dataset = ConcatDataset([test_dataset_with_targets, test_dataset_no_targets])
        ...
        # Pre-compute all model outputs once
        Y_r, Y_v, X_rv_proj, Y_true = self.feed_forward(test_loader)
        
        # now sweep PFA from 1e-6 up to 1.0 (logarithmically)
        # 10 points logarithmically from 1e-6 to 1e-1
        target_pFAs = np.logspace(-6, -1, 10)
        results = []
        for target_pfa in tqdm(target_pFAs, desc="Testing PFAs"):
            th = self.find_threshold(test_loader, target_pfa)
            Y_hat  = self.predict(Y_r, Y_v, X_rv_proj, th)
            mets   = self.get_metrics(Y_hat, Y_true)
            results.append((mets["Pd"], mets["Pfa"]))
    ...
    def evaluate_pd_scnr(self, nu: float, target_pfa: float = 5e-4) -> tuple:
        """Evaluate PD vs SCNR for fixed PFA"""
        # First find threshold using a reference dataset (SCNR = 0)
        ref_dataset_with_targets = RadarDataset(4096, n_targets=4, random_n_targets=False, nu=nu, snr=10, cnr=15)
        ref_dataset_no_targets = RadarDataset(2048, n_targets=0, random_n_targets=False, nu=nu, snr=10, cnr=15)
        ref_dataset = ConcatDataset([ref_dataset_with_targets, ref_dataset_no_targets])
        ref_loader = DataLoader(ref_dataset, batch_size=256, shuffle=True, num_workers=2,
                              pin_memory=torch.cuda.is_available(), persistent_workers=True)
        ...
        for scnr in tqdm(scnr_range, desc="Testing SCNR values"):
            # Create dataset for this SCNR
            test_dataset_with_targets = RadarDataset(4096, n_targets=4, random_n_targets=False, nu=nu, snr=10, cnr=15)
            test_dataset_no_targets = RadarDataset(2048, n_targets=0, random_n_targets=False, nu=nu, snr=10,  cnr=15)
            test_dataset = ConcatDataset([test_dataset_with_targets, test_dataset_no_targets])
            test_loader = DataLoader(test_dataset, batch_size=256, shuffle=True, num_workers=2,
                              pin_memory=torch.cuda.is_available(), persistent_workers=True)
